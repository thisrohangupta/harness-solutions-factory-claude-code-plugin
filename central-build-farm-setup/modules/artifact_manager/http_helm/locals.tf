locals {
  ...
  # Define valid authentication types for Helm Connector
  valid_helm_auth_types = ["UsernamePassword", "Anonymous"]

  # Validation for authentication type
  auth_type_valid = (
    contains(local.valid_helm_auth_types, var.artifact_manager_auth_type)
    ? null
    : "[Invalid] Chosen Helm authentication type '${var.artifact_manager_auth_type}' is not supported. Valid types: ${join(", ", local.valid_helm_auth_types)}"
  )

  # Validation for Helm Artifact Manager URL
  helm_url_valid = (
    length(var.artifact_manager_url) > 0
    ? null
    : "[Invalid] Missing value for 'artifact_manager_url', required for Helm connector."
  )
