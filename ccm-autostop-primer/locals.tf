locals {
  ...
  template_organization_id = (
    var.template_organization_id != null
    ?
    data.harness_platform_organization.template[0].id
    :
    null
  )

  template_project_id = (
    var.template_project_id != null
    ?
    data.harness_platform_project.template[0].id
    :
    null
  )
